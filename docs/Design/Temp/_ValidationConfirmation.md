...
- [ ] There are problems with generation, so currently the validator attribute arguments must be legal if passed directly by position to a constructor on the corresponding validator. We could change this to named arguments if we insist the names also match, but unless we can untangle the validator constructor parameters, and find the right constructor, we can't use the properties as named parameter feature.
- [ ] The validator attribute must have both a specific base class and an attribute. We might be able to drop the base class.
- 
- 
- ## Current syntax
